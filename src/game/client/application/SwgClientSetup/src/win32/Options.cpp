                        ...
                        case S_direct3d9:
                                {
                                        if (left == _T("maxPixelShaderVersion"))
                                        {
                                                ...
                                                else if (right.CompareNoCase (_T("0x0101")) == 0)
                                                {
                                                        ms_pixelShaderMajorVersion = 1;
                                                        ms_pixelShaderMinorVersion = 1;
                                                }
                                                else if (right.CompareNoCase (_T("0x0104")) == 0)
                                                {
                                                        ms_pixelShaderMajorVersion = 1;
                                                        ms_pixelShaderMinorVersion = 4;
                                                }
                                                else if (right.CompareNoCase (_T("0x0200")) == 0)
                                                {
                                                        ms_pixelShaderMajorVersion = 2;
                                                        ms_pixelShaderMinorVersion = 0;
                                                }
                                                else if (right.CompareNoCase (_T("0x0300")) == 0)
                                                {
                                                        ms_pixelShaderMajorVersion = 3;
                                                        ms_pixelShaderMinorVersion = 0;
                                                }
                                                else if (right.CompareNoCase (_T("0xffff")) == 0)
                                                {
                                                        ms_pixelShaderMajorVersion = -1;
                                                        ms_pixelShaderMinorVersion = -1;
                                                }
...
void Options::upgradeForModernHardware()
{
        ...
        if (supportedPixelShaderMajor >= 3)
        {
        ...
        const int physicalMemorySize = ClientMachine::getPhysicalMemorySize();
        const int videoMemorySize = ClientMachine::getVideoMemorySize();

        if (physicalMemorySize >= 512)
        {
                ms_discardHighestMipMapLevels = false;
                ms_discardHighestNormalMipMapLevels = false;
        }

        if ((recommendedPixelShaderMajor >= 1 && videoMemorySize >= 128) || recommendedPixelShaderMajor >= 2)
        {
                ms_disableBumpMapping = false;
        }

        if (recommendedPixelShaderMajor >= 1 && ((recommendedPixelShaderMajor >= 2 && videoMemorySize >= 192) || videoMemorySize >= 256))
        {
                ms_disableMultipassRendering = false;
        }

        if (videoMemorySize >= 256)
                ms_disableTextureBaking = false;

        if (ClientMachine::getSupportsHardwareMouseCursor())
                ms_disableHardwareMouseCursor = false;

        if (recommendedPixelShaderMajor >= 2 || (recommendedPixelShaderMajor >= 1 && videoMemorySize >= 256))
...
bool Options::save ()
{
	...
	if (ms_allowTearing || ms_pixelShaderMajorVersion != -1  || ms_fullScreenRefreshRate > 0)
        ...
        {
                if (ms_pixelShaderMajorVersion >= 3)
                {

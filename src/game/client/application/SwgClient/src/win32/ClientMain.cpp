	...
	)
{
	...
	// Remove or bypass the semaphore check
	// Skip the semaphore code completely to allow multiple instances
	// SetLastError(0);
	// HANDLE semaphore = CreateSemaphore(NULL, 0, 1, "Swg+ClientInstanceRunning");
	// if (GetLastError() == ERROR_ALREADY_EXISTS && !ConfigFile::getKeyBool("Swg+Client", "allowMultipleInstances", PRODUCTION ? false : true))
	// {
	...
	//     if (semaphore)
	//         CloseHandle(semaphore);  // Ensure handle is closed to avoid memory leaks
	//     return 1;
	// }

	// Skip semaphore checks to allow multiple instances to run

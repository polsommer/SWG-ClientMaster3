...
namespace Direct3d9Namespace
{
	...
	void                               lostDevice();
        ...
        bool                               isGdiVisible();
        bool                               wasDeviceReset();
        bool                               isDirect3d9ExRuntimeAvailable();
        bool                               isUsingDirect3d9Ex();
	...
	void                               getOtherAdapterRects(std::vector<RECT> &otherAdapterRects);
        ...
        AdapterCandidate                   selectBestAdapterCandidate();
        AdapterCandidate                   buildAdapterCandidate(UINT adapterIndex);
        bool                               isBetterAdapterCandidate(const AdapterCandidate &candidate, const AdapterCandidate &current);
        int                                getVendorPriority(const AdapterCandidate &candidate);
        int                                computeShaderCapabilityFromCaps(const D3DCAPS9 &caps);
        bool                               isAmdVendor(DWORD vendorId);
        bool                               isNvidiaVendor(DWORD vendorId);
        bool                               isIntelVendor(DWORD vendorId);
        const char *                       describeVendor(DWORD vendorId);
	...
	void                               releaseBackBuffer();
        ...
        void                               applyDirect3d9ExConfiguration();
...
//	VectorRgba const &                 getAlphaFadeAndBloomSettings();
	...
	const char                                *ms_appearanceName;
        ...
        Direct3d9ExSupport::RuntimeHandles         ms_direct3d9ExRuntime;
...
Direct3d9Namespace::AdapterCandidate Direct3d9Namespace::buildAdapterCandidate(UINT adapterIndex)
{
        AdapterCandidate candidate;
        candidate.adapterIndex = adapterIndex;
        ...
        if (FAILED(ms_direct3d->GetDeviceCaps(adapterIndex, D3DDEVTYPE_HAL, &candidate.caps)))
                return candidate;

        candidate.valid = true;
        candidate.shaderCapability = computeShaderCapabilityFromCaps(candidate.caps);
        candidate.supportsHardwareVertexProcessing = (candidate.caps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT) != 0;
        candidate.isAmdAdapter = isAmdVendor(candidate.identifier.VendorId);
        candidate.isNvidiaAdapter = isNvidiaVendor(candidate.identifier.VendorId);
        candidate.isIntelAdapter = isIntelVendor(candidate.identifier.VendorId);
...
Direct3d9Namespace::AdapterCandidate Direct3d9Namespace::selectBestAdapterCandidate()
{
        ...
        {
                AdapterCandidate candidate = buildAdapterCandidate(adapterIndex);
                if (isBetterAdapterCandidate(candidate, bestCandidate))
...
bool Direct3d9Namespace::isBetterAdapterCandidate(const AdapterCandidate &candidate, const AdapterCandidate &current)
{
        ...
        if (candidate.shaderCapability != current.shaderCapability)
                return candidate.shaderCapability > current.shaderCapability;

        const int candidateVendorPriority = getVendorPriority(candidate);
        const int currentVendorPriority = getVendorPriority(current);
        if (candidateVendorPriority != currentVendorPriority)
                return candidateVendorPriority > currentVendorPriority;

        if (candidate.supportsHardwareVertexProcessing != current.supportsHardwareVertexProcessing)
                return candidate.supportsHardwareVertexProcessing;

        return candidate.adapterIndex < current.adapterIndex;
}

// ----------------------------------------------------------------------

int Direct3d9Namespace::getVendorPriority(const AdapterCandidate &candidate)
{
        if (candidate.isAmdAdapter || candidate.isNvidiaAdapter)
                return 3;
        if (candidate.isIntelAdapter)
                return 1;
        return 2;
}
...
int Direct3d9Namespace::computeShaderCapabilityFromCaps(const D3DCAPS9 &caps)
{
#ifdef VSPS
        if (!ConfigDirect3d9::getDisableVertexAndPixelShaders() && caps.VertexShaderVersion >= 0x0200 && caps.PixelShaderVersion >= 0x0200)
                return ShaderCapability(2,0);
        if (!ConfigDirect3d9::getDisableVertexAndPixelShaders() && caps.VertexShaderVersion >= 0x0101 && caps.PixelShaderVersion >= 0x0104)
                return ShaderCapability(1,4);
        if (!ConfigDirect3d9::getDisableVertexAndPixelShaders() && caps.VertexShaderVersion >= 0x0101 && caps.PixelShaderVersion >= 0x0101)
        ...
        if (caps.MaxSimultaneousTextures >= 2)
                return ShaderCapability(0,2);
        return ShaderCapability(0,0);
#else
        DEBUG_FATAL(true, ("VSPS-only graphics layer selected but no VSPS supported"));
        return ShaderCapability(1,1, true);
#endif
}

// ----------------------------------------------------------------------

bool Direct3d9Namespace::isAmdVendor(DWORD vendorId)
{
...
bool Direct3d9Namespace::isNvidiaVendor(DWORD vendorId)
{
        return vendorId == 0x10deu;
}

// ----------------------------------------------------------------------

bool Direct3d9Namespace::isIntelVendor(DWORD vendorId)
{
        return vendorId == 0x8086u;
}

// ----------------------------------------------------------------------

const char *Direct3d9Namespace::describeVendor(DWORD vendorId)
{
        if (isAmdVendor(vendorId))
                return "AMD";
        if (isNvidiaVendor(vendorId))
                return "NVIDIA";
        if (isIntelVendor(vendorId))
                return "Intel";
        return "Unknown";
}
...
bool Direct3d9::install(Gl_install *gl_install)
{
	...
	DEBUG_FATAL(ms_installed, ("already installed"));
        ...
        ms_direct3d = NULL;
        ms_direct3dEx = NULL;
        Zero(ms_direct3d9ExRuntime);
        ms_device = NULL;
        ms_deviceEx = NULL;
        ms_usingDirect3d9Ex = false;
        ms_waitForVBlankAfterPresent = ConfigDirect3d9::getWaitForVBlankAfterPresent();
        ms_waitForVBlankAdapter = ConfigDirect3d9::getWaitForVBlankAdapter();
        CrashReportInformation::addStaticText("Direct3D9WaitForVBlankAfterPresent: %d\n", ms_waitForVBlankAfterPresent ? 1 : 0);
        CrashReportInformation::addStaticText("Direct3D9WaitForVBlankAdapter: %d\n", ms_waitForVBlankAdapter);
	...
	ms_glApi.isDirect3d9ExRuntimeAvailable     = isDirect3d9ExRuntimeAvailable;
	ms_glApi.isUsingDirect3d9Ex                = isUsingDirect3d9Ex;
	...
	ms_glApi.releaseVideoBuffers = releaseVideoBuffers;
        ...
        // -----------------------------------
        // create Direct3d
        if (ConfigDirect3d9::getPreferDirect3d9Ex())
        {
                if (Direct3d9ExSupport::acquireRuntime(true, &ms_direct3d9ExRuntime))
                {
                        HRESULT const createResult = Direct3d9ExSupport::createInterface(ms_direct3d9ExRuntime, D3D_SDK_VERSION, &ms_direct3dEx);
                        if (SUCCEEDED(createResult) && ms_direct3dEx)
                        {
                                ms_direct3d = ms_direct3dEx;
                                ms_usingDirect3d9Ex = true;
                                CrashReportInformation::addStaticText("Direct3D9Runtime: 9Ex\n");
                        }
                        else
                        {
                                CrashReportInformation::addStaticText("Direct3D9ExFallback: CreateInterface 0x%08x\n", static_cast<unsigned int>(createResult));
                                Direct3d9ExSupport::releaseRuntime(ms_direct3d9ExRuntime);
                                Zero(ms_direct3d9ExRuntime);
                                ms_direct3dEx = NULL;
                        }
                }
                ...
                {
                        CrashReportInformation::addStaticText("Direct3D9ExFallback: RuntimeUnavailable\n");
                }
        }
        ...
        if (!ms_direct3d)
        {
                ...
                CrashReportInformation::addStaticText("Direct3D9Runtime: Classic\n");
        }
        // -----------------------------------
	...
	// ---------------------------------------------------------------------------------
        ...
        {
                AdapterCandidate autoSelectedAdapter;
                bool usedAutoSelection = false;
                ...
                const int numberOfAdapters = ms_direct3d->GetAdapterCount();
                if (ms_adapter >= static_cast<UINT>(numberOfAdapters) && numberOfAdapters > 0)
                {
                        REPORT_LOG(verboseHardwareLogging, ("Requested adapter %u is out of range, falling back to default adapter.\n", ms_adapter));
                        CrashReportInformation::addStaticText("D3dInvalidAdapter: %u/%d\n", ms_adapter, numberOfAdapters);
                        ms_adapter = D3DADAPTER_DEFAULT;
                        usedAutoSelection = false;
                }

                REPORT_LOG(verboseHardwareLogging, ("Using adapter %u of %d:\n", ms_adapter, numberOfAdapters));
                CrashReportInformation::addStaticText("VideoAdapter: %u/%d\n", ms_adapter, numberOfAdapters);

                if (usedAutoSelection)
                {
                        const char *const vendorName = describeVendor(autoSelectedAdapter.identifier.VendorId);
                        REPORT_LOG(verboseHardwareLogging, ("Auto-selected adapter %u (%s, device 0x%04x)\n", autoSelectedAdapter.adapterIndex, vendorName, autoSelectedAdapter.identifier.DeviceId));
                        CrashReportInformation::addStaticText("D3dAutoAdapter: %u (%s)\n", autoSelectedAdapter.adapterIndex, vendorName);
                }
                ...
                const char *const adapterVendor = describeVendor(ms_adapterIdentifier.VendorId);
                ...
                REPORT_LOG(verboseHardwareLogging, ("Adapter vendor: %s\n", adapterVendor));
                ...
                CrashReportInformation::addStaticText("D3dVendor: %s\n", adapterVendor);
        }
        // ---------------------------------------------------------------------------------
						...
						SUCCEEDED(ms_direct3d->CheckDepthStencilMatch(ms_adapter, ms_deviceType, ms_adapterFormat, ms_backBufferFormat, ms_depthStencilFormat)))
				{
					...
					if (vertexProcessingMode == D3DCREATE_HARDWARE_VERTEXPROCESSING && ConfigDirect3d9::getUsePureDevice())
						vertexProcessingMode |= D3DCREATE_PUREDEVICE;
                                        ...
                                        if (ms_usingDirect3d9Ex && ms_direct3dEx)
                                        {
                                                IDirect3DDevice9Ex *deviceEx = NULL;
                                                D3DDISPLAYMODEEX displayModeEx;
                                                D3DDISPLAYMODEEX *displayModePtr = NULL;

                                                if (!ms_presentParameters.Windowed)
                                                {
                                                        Zero(displayModeEx);
                                                        displayModeEx.Size = sizeof(displayModeEx);
                                                        displayModeEx.Width = ms_presentParameters.BackBufferWidth;
                                                        displayModeEx.Height = ms_presentParameters.BackBufferHeight;
                                                        displayModeEx.Format = ms_presentParameters.BackBufferFormat;
                                                        displayModeEx.RefreshRate = ms_presentParameters.FullScreen_RefreshRateInHz ? static_cast<UINT>(ms_presentParameters.FullScreen_RefreshRateInHz) : static_cast<UINT>(ms_displayMode.RefreshRate);
                                                        displayModeEx.ScanLineOrdering = D3DSCANLINEORDERING_PROGRESSIVE;
                                                        displayModePtr = &displayModeEx;
                                                }

                                                hresult = ms_direct3dEx->CreateDeviceEx(ms_adapter, ms_deviceType, ms_window, vertexProcessingMode, &ms_presentParameters, displayModePtr, &deviceEx);
                                                if (SUCCEEDED(hresult))
                                                {
                                                        ms_deviceEx = deviceEx;
                                                        ms_device = deviceEx;
                                                        applyDirect3d9ExConfiguration();
                                                }
                                                else
                                                {
                                                        CrashReportInformation::addStaticText("Direct3D9ExFallback: CreateDeviceEx 0x%08x\n", static_cast<unsigned int>(hresult));

                                                        if (ms_deviceEx)
                                                        {
                                                                IGNORE_RETURN(ms_deviceEx->Release());
                                                                ms_deviceEx = NULL;
                                                                ms_device = NULL;
                                                        }
...
void Direct3d9Namespace::remove()
{
	...
	releaseBackBuffer();
        ...
        ms_direct3dEx = NULL;
        if (ms_direct3d9ExRuntime.module || ms_direct3d9ExRuntime.createProc)
        {
                Direct3d9ExSupport::releaseRuntime(ms_direct3d9ExRuntime);
                Zero(ms_direct3d9ExRuntime);
        }
...
bool Direct3d9Namespace::isDirect3d9ExRuntimeAvailable()
{
...
bool Direct3d9Namespace::isUsingDirect3d9Ex()
{
...
void Direct3d9Namespace::applyDirect3d9ExConfiguration()
{
        if (!ms_usingDirect3d9Ex || !ms_deviceEx)
                return;

        UINT const latency = Direct3d9ExSupport::clampMaximumFrameLatency(static_cast<UINT>(ConfigDirect3d9::getMaximumFrameLatency()));
        HRESULT const latencyResult = Direct3d9ExSupport::setMaximumFrameLatency(ms_deviceEx, latency);
        if (SUCCEEDED(latencyResult))
                CrashReportInformation::addStaticText("Direct3D9ExFrameLatency: %u\n", latency);
        else
                CrashReportInformation::addStaticText("Direct3D9ExFrameLatencyError: 0x%08x\n", static_cast<unsigned int>(latencyResult));

        INT const requestedPriority = ConfigDirect3d9::getGpuThreadPriority();
        HRESULT const priorityResult = Direct3d9ExSupport::setGpuThreadPriority(ms_deviceEx, requestedPriority);
        if (SUCCEEDED(priorityResult))
        {
                INT effectivePriority = requestedPriority;
                if (SUCCEEDED(Direct3d9ExSupport::getGpuThreadPriority(ms_deviceEx, &effectivePriority)))
                        CrashReportInformation::addStaticText("Direct3D9ExGpuThreadPriority: %d\n", effectivePriority);
        }
        else
        {
                CrashReportInformation::addStaticText("Direct3D9ExGpuThreadPriorityError: 0x%08x\n", static_cast<unsigned int>(priorityResult));
        }
}
...
bool Direct3d9Namespace::present(bool windowed, HWND window, int width, int height)
{
	...
	else
	{
        ...
        if (SUCCEEDED(hresult) && ms_usingDirect3d9Ex && ms_deviceEx && ms_waitForVBlankAfterPresent)
        {
                UINT const adapterIndex = ms_waitForVBlankAdapter >= 0 ? static_cast<UINT>(ms_waitForVBlankAdapter) : ms_adapter;
                IGNORE_RETURN(Direct3d9ExSupport::waitForVBlank(ms_deviceEx, adapterIndex));
        }

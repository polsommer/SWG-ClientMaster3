...
namespace
{
	...
	bool shouldUseTresToken(char const *fileName)
	{
		if (!fileName)
			return false;
		...
		if (!extension || *(extension + 1) == '\0')
			return false;
		return stringsEqualIgnoreCase(extension + 1, "tres");
	}
...
static void run(void)
{
        ...
        if (mc != CommandLine::MC_MATCH)
	{
        ...
        bool passphraseProvided = TreeFileEncryption::isPassphraseValid(passphrase.c_str());
        ...
        if (decryptMode)
        {
                ...
                {
                        fprintf(stderr, "Decryption requires a non-empty passphrase. Use --%s or configure SharedFile.treeFileEncryptionPassphrase.\n", LNAME_PASSPHRASE);
                        ++errors;
                        return;
                }
        ...
        bool encryptionRequested = shouldUseTresToken(outputTreeFileName);
        ...
        {
                fprintf(stderr, "Encryption requires a non-empty passphrase. Use --%s or configure SharedFile.treeFileEncryptionPassphrase.\n", LNAME_PASSPHRASE);
                ++errors;
                return;
        }
...
static void printConfigurationSummary(const char *outputTreeFileName, const char *responseFile, bool encryptionRequested, bool passphraseProvided)
{
        ...
        logDiagnostics("  Encryption       : %s", encryptionRequested ? (passphraseProvided ? "enabled" : "requested (missing passphrase)") : "disabled");
...
static void printDecryptionSummary(const char *source, const char *destination, bool passphraseProvided)
{
        ...
        logDiagnostics("  Passphrase  : %s", passphraseProvided ? "provided" : "missing");
}
...
static bool runInteractiveBuildWorkflow(void)
{
        ...
        std::string outputPrompt = std::string("Output tree file [") + defaultOutput + "]: ";
        ...
        bool const encryptionDefault = shouldUseTresToken(outputTreeFile.c_str());
        ...
        if (encryptionRequested && !passphraseProvided)
        {
                printf("A non-empty passphrase is required when encryption is enabled.\n");

                std::string passphraseInput;
                for (;;)
                {
                        if (!promptForLine("Enter passphrase: ", false, passphraseInput))
                                return false;

                        if (TreeFileEncryption::isPassphraseValid(passphraseInput.c_str()))
                        {
                        ...
                        printf("Passphrase cannot be empty. Please try again.\n");
                }
        }
        ...
        printf("  Encryption    : %s\n", encryptionRequested ? (passphraseProvided ? "enabled" : "requested (missing passphrase)") : "disabled");
...
static bool runInteractiveDecryptWorkflow(void)
{
        ...
        if (!promptForExistingFile("Encrypted tree file (.tres): ", sourceFile))
                return false;
        ...
        {
                std::string passphraseInput;
                printf("A passphrase is required to decrypt the file.\n");
...
void TreeFileBuilder::write(void)
{
        // write the header
        TreeFile::SearchTree::Header header;
        header.token         = shouldUseTresToken(treeFileName) ? TAG_TRES : TAG_TREE;
...
bool TreeFileBuilder::encryptTreeFile(const char *sourceFileName, const char *destinationFileName, const char *passphrase)
{
        if (!TreeFileEncryption::isPassphraseValid(passphrase))
        {
                fprintf(stderr, "Encryption requires a non-empty passphrase.\n");
                return false;
        }
...
bool TreeFileBuilder::decryptTreeFile(const char *sourceFileName, const char *destinationFileName, const char *passphrase)
{
        if (!TreeFileEncryption::isPassphraseValid(passphrase))
        {
                fprintf(stderr, "Decryption requires a non-empty passphrase.\n");
                return false;
        }
